...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        #if state_ is corner but isn't the last added in the list
        #if state in corners and it is the last than we will return true
    ...
    def getNextState(self, state, action):
        ...
        #return tupple((x,y),[one list]])
        #the list have the corners where is explored this time
...
def cornersHeuristic(state, problem):
    ...
    #take node and visited state(corners)
    node, visited = state[0], list(state[1])
    min_path = 0
    while len(visited) != 4:
        
        #we find the min path from this state in one corner state
        #and after from this corner in the next corners (where corners isn't visited) until the size from visited corners is 4 
        min_dist = math.inf
        min_corner = None
        for corner_1 in corners:
            #if corner isn't visited
...
def foodHeuristic(state, problem):
    ...
    #if foodlist is zero return zero
    if foodList == []:
        return 0
    ...
    for food in foodList:
        ...
        #until foodlist isn't zero
        while len( fList) != 0:
            min_dist = math.inf
            min_state_food = None

            #from this for-loop we will find the min path from the node(food) in one other food
            for f in fList:
